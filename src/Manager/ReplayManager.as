namespace ReplayLoader {
    ...
    void LoadReplayFromPath(const string &in path, bool verbose = true) {
        ...
        if (verbose) log(task.ErrorCode, LogLevel::Info, 24, "LoadReplayFromPath");
        if (verbose) log(task.ErrorDescription, LogLevel::Info, 25, "LoadReplayFromPath");
        if (verbose) log(task.ErrorType, LogLevel::Info, 26, "LoadReplayFromPath");
        if (verbose) log(tostring(task.Ghosts.Length), LogLevel::Info, 27, "LoadReplayFromPath");
        ...
        if (verbose) log("Replay loaded successfully with " + task.Ghosts.Length + " ghosts!", LogLevel::Info, 34, "LoadReplayFromPath");
    }
}
